...
```{r gen_map_data, echo=FALSE, results='hide', cache=TRUE}
source('src/generate_map_data.R') # See presentation gitub for file src
...
# Read map ready data if file already exists
data_map <- readRDS("data/cal3yr_avg_api_map_data.rds")  
```

```{r map_data, fig.width=12, fig.height=4.5, cache=TRUE}
library(ggplot2)
library(grid)
